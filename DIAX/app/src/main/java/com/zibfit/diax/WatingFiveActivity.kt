...
class WatingFiveActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        waitingFiveLottie.setAnimation(R.raw.loading_ce1)
...
//format for minute  ----->  "Time Remaining %02d min: %02d sec"
    ...
    private fun shakeItBaby(context: Context) {
        if (Build.VERSION.SDK_INT >= 26) {
            (context.getSystemService(VIBRATOR_SERVICE) as Vibrator).vibrate(VibrationEffect.createOneShot(500, VibrationEffect.EFFECT_TICK))
        } else {
